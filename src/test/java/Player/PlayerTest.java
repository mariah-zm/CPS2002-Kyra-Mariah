...
public class PlayerTest {
    ...
    @Test
    public void addDiscoveredTile_NoDuplicates() {
        int x = player.getInitial().getX();
        int y = player.getInitial().getY();

        Tile tile = player.getMap().getTile(x, y);
        player.addDiscoveredTile(tile);

        int size = player.getDiscoveredTiles().size();

        assertEquals(1, size);
    }

    @Test
    public void addToTeam_IDUpdated() {
        Team team = new Team(1);
        player.addToTeam(team);

        assertTrue(player.getPlayerID().contains(team.getTeamID()));
    }

    @Test
    public void addToTeam_SubjectUpdated() {
        Team team = new Team(1);
        player.addToTeam(team);

        assertEquals(team, player.subject);
    }

    @Test
    public void addToTeam_AddedToObserversList(){
        Team team = new Team(1);
        player.addToTeam(team);

        assertTrue(team.getObservers().contains(player));
    }
    ...
    public void update(){
        ...
        int newX = test_position.getX();
        int newY = test_position.getY();

        Tile newTile = map.getTile(newX, newY);
        team.setSubjectLastExplored(newTile);

        assertTrue(player.getDiscoveredTiles().contains(newTile));
    }
